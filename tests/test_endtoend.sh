...
# ---------------------------------------------------------------------------
# Every selectable method, run as far as superclusters. Figures and the report
# are skipped because they exercise nothing method-specific and cost most of
# the runtime.
#
# This section exists because auto_label: shape_simple was broken for its
# whole life and nothing noticed: it is not on the default path, and the demo
# never selects it. A config option nothing runs is an option that does not
# work.
alt_run () {
    local name="$1" extra="$2" dir="$SANDBOX/alt_$1"
    { sed "s#^  dir: .*#  dir: $dir#" "$CFG"; printf "%s\n" "$extra"; } > "$SANDBOX/alt_$name.yaml"
    if "${RUN[@]}" snakemake --configfile "$SANDBOX/alt_$name.yaml" -j 4 \
            --until superclusters > "$SANDBOX/alt_$name.log" 2>&1; then
        local arms_out
        arms_out=$(find "$dir/clusters" -name '*_clusters.tsv' 2>/dev/null | wc -l)
        if (( arms_out > 0 )); then
            pass "$name produced a cluster table"
        else
            fail "$name completed but wrote no cluster table"
        fi
    else
        fail "$name failed (see $SANDBOX/alt_$name.log)"
        tail -5 "$SANDBOX/alt_$name.log"
    fi
}

# Each alternative caps max_features hard. These assert that a code path
# runs, not what it produces, and degPatterns cost is cubic: without the cap
# this section alone takes ten minutes.
alt_run wald_union   "differential:
  method: wald_union
  min_contrasts: 2
cluster:
  max_features: 600"
alt_run method_none  "differential:
  method: none
  top_n: 2000
cluster:
  max_features: 600"
alt_run shape_simple "superclusters:
  auto_label: shape_simple
  split: null
cluster:
  max_features: 600"
alt_run hclust       "superclusters:
  method: hclust
  k: 3
  split: null
cluster:
  max_features: 600"
alt_run minc_auto    "cluster:
  max_features: 600
  degpatterns:
    minc: auto"
alt_run subsample    "cluster:
  max_features: 400"
alt_run gene_mode    "input:
  features: null
cluster:
  max_features: 600"
